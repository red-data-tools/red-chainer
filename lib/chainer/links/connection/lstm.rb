...
class LSTMBase < Chainer::Chain
  def initialize(in_size, out_size: nil, lateral_init: nil
    ...
    init_scope do
      @upward = Chainer::Links::Connection::Linear.new(in_size, 4 * out_size, initial_w: 0)
      @lateral = Chainer::Links::Connection::Linear.new(out_size, 4 * out_size, initial_w: 0)
                                     nobias=True)
      self.initialize_params() if in_size
    end
  end

  def initialize_params
    lateral_init = Initializers.get_initializer(@lateral_init)
    upward_init = Initializers.get_initializer(@upward_init)

    Utils::Array.ndindex([0, 4 * @state_size, @state_size]).each do |i|
      lateral_init(@lateral.w.data[i..(i + @state_size), true])
      upward_init(@upward.w.data[i..(i + @state_size), true])
    end
