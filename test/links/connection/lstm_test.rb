...
module Chainer::Links::Connection
  class LSTMTest < Test::Unit::TestCase
    ...
    def _setup(data)
      ...
      @x2 = Numo::SFloat.new(x2_shape).rand(-1, 1)
      x3_shape = [0, data[:in_size]]
      @x3 = Numo::SFloat.new(x3_shape).rand(-1, 1)
    end
    ...
    def test_forward(data)
      ...
      h1 = @link.(x1)
      c0 = Chainer::Variable.new(@x1.class.zeros(@x1.shape[0], data[:out_size]))
      (c1_expect, h1_expect) = Chainer::Functions::Activation::LSTM.lstm(c0, @link.upward.(x1))
      ...
      batch = @x2.shape[0]
      x2 = data[:input_variable] ? Chainer::Variable.new(@x2) : @x2
      (h1_in, h1_rest) = Chainer::Functions::Array::SplitAxis.split_axis(@link.h.data, [batch], 0)
      y2 = @link.(x2)
      (c2_expect, y2_expect) = Chainer::Functions::Activation::LSTM.lstm(c1_expect, @link.upward.(x2) + @link.lateral.(h1_in))
      ...
      h2_rest = @link.h
      y3 = @link.(x3)
      (c3_expect, y3_expect) = Chainer::Functions::Activation::LSTM.lstm(c2_expect, @link.upward.(x3))
